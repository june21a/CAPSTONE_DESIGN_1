...
class SensorAgent(autonomous_agent.AutonomousAgent):
  ...
  def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
    ...
    if strtobool(os.environ.get('DISABLE_CUDNN', '0')):
      ...
      print('cuDNN disabled for evaluation.')
  ...
  def _tensor_rgb_to_numpy(rgb):
    ...
    return np.ascontiguousarray(np.clip(rgb_image, 0, 255).astype(np.uint8))

  @staticmethod
  def _normalize_map(attention_map):
    attention_map = attention_map.detach().float().cpu().numpy()
  ...
  def _save_sensor_attention_data(self, tick_data, lidar_bev, rgb_attention_map, lidar_attention_map, control,
    ...
    sensor_path = pathlib.Path(self.save_path) / 'sensor_data'
    rgb_path = sensor_path / 'rgb'
    ...
    cv2.imwrite(str(rgb_path / f'{frame_id}.png'), cv2.cvtColor(rgb_image, cv2.COLOR_RGB2BGR))
  ...
  def destroy(self, results=None):  # pylint: disable=locally-disabled, unused-argument
    ...
    if getattr(self, 'save_path', None) is not None:
      ...
      if getattr(self, 'config', None) is not None and self.config.tp_attention:
